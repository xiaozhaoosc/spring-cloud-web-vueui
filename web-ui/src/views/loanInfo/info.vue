<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input :placeholder="$t('table.fullname')" v-model="listQuery.customer_name" clearable style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
      ...
      <el-input :placeholder="$t('table.applicationId')" v-model="listQuery.application_id" clearable style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
      ...
      <el-select v-model="listQuery.status" :placeholder="$t('table.loanStatus')" clearable style="width: 140px" class="filter-item" @change="handleFilter">
        <el-option v-for="item in $store.state.financeVar.loanStatuses" :key="item.key" :label="item.display_name" :value="item.key"/>
      </el-select>
      <!-- <el-select v-model="listQuery.loanType" placeholder="借款类型" clearable style="width: 140px" class="filter-item" @change="handleFilter">
        <el-option v-for="item in $store.state.financeVar.loanTypes" :key="item.key" :label="item.display_name" :value="item.key"/>
      </el-select> -->
      <el-select v-model="listQuery.loanType" placeholder="贷种产品" clearable style="width: 140px" class="filter-item" @change="handleFilter">
        <el-option v-for="item in loanTypeOptions" :key="item.key" :label="item.display_name" :value="item.key"/>
      </el-select>
    </div>
    <div class="filter-container">
      <el-date-picker v-model="listQuery.start_time" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" class="filter-item" placeholder="开始日期"/>
      <el-date-picker v-model="listQuery.end_time" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" class="filter-item" placeholder="结束日期"/>
    ...
    <el-table
      ...
      <el-table-column :label="$t('table.loanStatus')" width="80px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="贷种产品" width="80px" align="center">
        <template slot-scope="scope">
          ...
          <el-card shadow="always">
            <el-form v-if="loanInfoDetail.loanStagesDto !== null" ref="dataForm" :inline="true" :model="temp" label-width="100px" style="margin-left:50px;">
              <el-form-item :label="'申请订单号'">
                <el-input v-model="temp.application_id" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'本金'">
                <el-input v-model="loanInfoDetail.loanStagesDto.principal" style="width:300px"/>
              </el-form-item>
              ...
              <el-form-item :label="'结清方式'">
                <el-tag style="width:300px">
                  <span v-if="loanInfoDetail.loanStagesDto.settleWay === 1">正常还款</span>
                  <span v-if="loanInfoDetail.loanStagesDto.settleWay === 2">提前还款</span>
                  <span v-if="loanInfoDetail.loanStagesDto.settleWay === 3">续期</span>
                </el-tag>
              </el-form-item>
              ...
              <el-form-item :label="'开始日期'">
                <el-input v-model="loanInfoDetail.loanStagesDto.beginDate" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'结束日期'">
                <el-input v-model="loanInfoDetail.loanStagesDto.endDate" style="width:300px"/>
              </el-form-item>
            </el-form>
          </el-card>
        ...
        <el-tab-pane label="代付">
          <el-card :model="temp" shadow="always">
            <el-table
              ...
              <el-table-column label="申请订单号" align="center">
                <template slot-scope="scope">
                  <span>{{ temp.application_id }}</span>
                </template>
              </el-table-column>
              ...
              <el-table-column label="订单完成时间" align="center" >
                <template slot-scope="scope">
                  <span>{{ scope.row.orderFinishTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
        ...
        <el-tab-pane label="代扣">
          <el-card :model="temp" shadow="always">
            <el-table
              ...
              <el-table-column label="订单完成时间" align="center" >
                <template slot-scope="scope">
                  <span>{{ scope.row.loanThirdTime | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
              <el-table-column label="备注" align="center" >
                <template slot-scope="scope">
        ...
        <el-tab-pane label="续期">
          <el-card :model="temp" shadow="always">
            <el-form v-if="loanInfoDetail.loanOrderRenewal != null" ref="dataForm" :inline="true" :model="temp" label-width="100px" style="margin-left:50px;">
              ...
              <el-form-item :label="'应还服务费'">
                <el-input v-model="loanInfoDetail.loanOrderRenewal.returnServiceCharge" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'续期已换本金'">
                <el-input v-model="loanInfoDetail.loanOrderRenewal.returnPrincipal" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'已换逾期费'">
                <el-input v-model="loanInfoDetail.loanOrderRenewal.returnOverdueCharge" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'续期已还利息'">
                <el-input v-model="loanInfoDetail.loanOrderRenewal.returnInterest" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'续期手续费'" >
                <el-input v-model="loanInfoDetail.loanOrderRenewal.renewalFee" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'续期应交保费'" >
                <el-input v-model="loanInfoDetail.loanOrderRenewal.returnSafe" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'剩余保费'" >
              ...
              <el-form-item :label="'续期申请时间'" >
                <el-input v-model="loanInfoDetail.loanOrderRenewal.applyTime" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'续期开始日期'" >
                <el-input v-model="loanInfoDetail.loanOrderRenewal.renewalStartdate" style="width:300px"/>
              </el-form-item>
              <el-form-item :label="'续期结束日期'" >
                <el-input v-model="loanInfoDetail.loanOrderRenewal.renewalEnddate" style="width:300px"/>
              </el-form-item>
    ...
    <el-dialog :title="'手动还款'" :visible.sync="dialogHandRepaymentVisible" width="550px">
      <el-form ref="dataFormRepayment" :inline="true" :model="repaymentLine" label-width="100px" style="margin-left:50px;">
        ...
        <el-form-item
          :label="'还款金额'"
          :rules="[
            { pattern: /^\d+(\.\d{0,2})?$/, required: true, trigger: 'manual',message: '还款金额有误!' }
          ]"
          prop="reAmount">
...
export default {
  ...
  methods: {
    ...
    handRepaymentView(row) {
      ...
      this.repaymentLine.payType = null
    ...
    reAmountHandleFilter() {
      ...
      if (this.repaymentLine.mhAmount === 'NaN') {
        this.repaymentLine.reAmount = 0
        this.repaymentLine.mhAmount = 0
      }
    },
    // 手动还款
    handRepayment(row) {
