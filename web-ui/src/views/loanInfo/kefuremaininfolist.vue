  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column :label="$t('table.actions')" align="center" width="190" class-name="small-padding fixed-width">
        ...
        <template slot-scope="scope">
          <el-button v-permission="['handRepayment']" v-if="scope.row.loan_type==1 && (scope.row.status == 5 || scope.row.status == 7)" type="warning" size="mini" @click="handRepaymentView(scope.row)">手动还款</el-button>
          <el-button v-permission="['handRepayment']" v-if="scope.row.loan_type==3 && (scope.row.status == 5 || scope.row.status == 7)" type="warning" size="mini" @click="handRepaymentView(scope.row)">手动还款</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { fetchListloanList, fetchStageInfo, fetchTradeInfo, loanInfoExport, getLoanInfoDetail, getRemainAmount, handRepayment } from '@/api/loanInfo'
import waves from '@/directive/waves'
import { parseTime } from '@/utils'
import { mathSubtr } from '@/utils/util'
export default {
  name: 'LoanInfo',
  directives: {
    waves
  ...
  methods: {
    getList() {
      ...
      fetchListloanList(this.listQuery).then(response => {
    ...
    reAmountHandleFilter() {
      ...
      this.repaymentLine.mhAmount = mathSubtr(this.repaymentLine.amount, this.repaymentLine.reAmount)// mathSub(Number(this.repaymentLine.amount), Number(this.repaymentLine.reAmount))

      this.$nextTick(() => {
        if (isNaN(this.repaymentLine.mhAmount)) {
          this.repaymentLine.reAmount = 0
